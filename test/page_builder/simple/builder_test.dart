...
void main() {
  ...
  testWidgets('route to page1 then second page -> second page is shown, deactivated is called', (WidgetTester tester) async {
    await tester.pumpWidget(SimpleBuilderApp());
    var presenter = AlfreedUtils.getPresenterByKey<MyPresenter, MyModel>(tester, ValueKey("presenter"));
  ...
  testWidgets('go second page add todo, then pop page, go back to second page -> second page is on initial state', (WidgetTester tester) async {
    await tester.pumpWidget(SimpleBuilderApp());
    ...
    await tester.tap(find.byType(IconButton).first);
    await tester.pumpAndSettle(Duration(milliseconds: 100));
    var presenter2 = AlfreedUtils.getPresenterByKey<MyPresenter, MyModel>(tester, ValueKey("presenter"));
    expect(find.text('second page'), findsOneWidget);
    expect(presenter2.state!.todoList!.length, equals(4));
  });

  testWidgets('route to page1 then second page -> second page is shown using arguments', (WidgetTester tester) async {
    await tester.pumpWidget(SimpleBuilderApp());
    var secondPageButton = find.byType(IconButton);
    await tester.tap(secondPageButton.first);
    await tester.pumpAndSettle(Duration(milliseconds: 100));
  ...
  testWidgets('add todo, then push page3, go back to second page -> second page is on previous state, onInit is not called again', (WidgetTester tester) async {
    await tester.pumpWidget(SimpleBuilderApp());
    // add a todo
    var presenterPage1 = AlfreedUtils.getPresenterByKey<MyPresenter, MyModel>(tester, ValueKey("presenter"));
    var floatingFinder = find.byType(FloatingActionButton);
    await tester.tap(floatingFinder);
    await tester.pumpAndSettle(Duration(milliseconds: 100));
    ...
    await tester.pumpAndSettle(Duration(milliseconds: 100));
    // pop page3
    var presenterPage3 = AlfreedUtils.getPresenterByKey<MyPresenter, MyModel>(tester, ValueKey("presenter3"));
    presenterPage3.view.pop();
    // page one has still 5 todo
    expect(presenterPage1.state!.todoList!.length, equals(5));
  });

  testWidgets('page has rebuildIfDisposed=false, go page add todo, then pop page, go back to page -> page has restored previous state',
